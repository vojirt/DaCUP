...
class BDD100k_2class(data.Dataset):
    NUM_CLASSES = 2 
    ...
    def __init__(self, cfg, root, split):
        ...
        self.images_base = os.path.join(self.root, "seg", "images", self.split)
        self.annotations_base = os.path.join(self.root, "seg", "labels", self.split) 

        self.files[split] = self.recursive_glob(rootdir=self.images_base, suffix='.jpg')

        self.void_classes = [10, 255] 
...
def main():
    ...
    cfg.DATASET = CN()
    cfg.DATASET.ROAD_ROI_ONLY = True

    bdd_train = BDD100k_2class(cfg, root="/mnt/datasets/BerkeleyDeepDriveDataset/bdd100k/", split='train')

    dataloader = DataLoader(bdd_train, batch_size=2, shuffle=True, num_workers=2)
